...
FROM 
	(SELECT Status,Transaction_Amount,Bank_Name, Card_Network,Payment_Channel,country_name
	FROM [ShopNow Transactions from Oct 2021 to March 2022_edit]
	WHERE Status = 'abandoned') AS abandoned_table
...
FROM 
 ...
 -- Value of permanently abandoned transactions
 -- Create table
 SELECT Status,Transaction_Amount,Bank_Name, Card_Network,Payment_Channel,country_name, Customer_ID
 INTO #stable
FROM [ShopNow Transactions from Oct 2021 to March 2022_edit]
...
SELECT #atable.Customer_ID, #atable.Status, #atable.Transaction_Amount
FROM #atable LEFT JOIN #stable ON #atable. Customer_ID = #stable.Customer_ID
WHERE #stable.Customer_ID IS NULL

-- total value

SELECT SUM(Transaction_Amount)
FROM  (SELECT #atable.Customer_ID, #atable.Status, #atable.Transaction_Amount
FROM #atable LEFT JOIN #stable ON #atable. Customer_ID = #stable.Customer_ID
WHERE #stable.Customer_ID IS NULL) AS SATABLE